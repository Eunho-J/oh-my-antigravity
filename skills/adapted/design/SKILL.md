...
Source: staged design migration copy.
...
- A feature needs `DESIGN.md` created or refreshed before an Antigravity implementation lane, designer lane, or execution workflow proceeds.

## Do not use when

- The user provides or requests a visual reference/image/live URL and wants measured implementation until screenshots match. Use a dedicated Antigravity visual implementation loop for that visual-reference implementation work.
- The task is pure backend/API/infrastructure work with no user-facing design consequence.
- The user only asks to compare screenshots or score visual fidelity. Use a dedicated Antigravity visual verdict flow.

## Relationship to visual implementation workflows
...
A visual implementation workflow owns implementation against an approved generated/static/live-URL visual reference, with screenshot capture, visual verdict scoring, and pixel-diff evidence. It may read `DESIGN.md`, and it may leave design-system artifacts behind, but it does not replace the `DESIGN.md` discovery/interview/refresh workflow.

If both are needed, run this design workflow first to establish the design contract, then run the visual implementation workflow only after the visual reference/baseline is approved.
...
- Screenshots, mockups, brand files, logos, Figma/export notes, Storybook snapshots, Playwright screenshots, visual-regression baselines, or `.oma/artifacts/visual/*` references.
...
- For visual-reference/image/live-URL matching, hand off to a visual implementation workflow with the approved reference/baseline and note that `DESIGN.md` is supporting context, not the visual verdict target.
...
- Any visual implementation handoff is clearly separated as visual implementation matching, not DESIGN.md governance.
