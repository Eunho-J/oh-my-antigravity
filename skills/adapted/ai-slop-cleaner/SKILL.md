...
Source: staged ai-slop-cleaner migration copy
...
## Antigravity Guidance Alignment
...
## Scoped File Lists and Persistent Workflow
...
- In persistent Antigravity workflows, the mandatory deslop pass should run this skill on the changed-files scope only, in standard mode unless the caller explicitly requests otherwise.
...
3. **Inventory fallback-like code before editing**
   ...
   - For broad, ambiguous, cross-layer, or architectural fallback-like code, open an explicit planning/consensus thread before edits
   - Recursion guard: when already inside another OMA or Antigravity workflow, do not spawn a nested planning workflow; record the finding and attach it to the active leader, plan, or handoff instead
